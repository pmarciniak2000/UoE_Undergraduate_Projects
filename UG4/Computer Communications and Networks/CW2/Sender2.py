...
def send(sock,packets,timeout,host,port):
    ...
    while current_packet < len(packets):
        ...
        while next_packet < current_packet + 1: # +1 for the window size which for stop-wait protcol is just 1
...
def recv_ack(sock):
    ...
    while receiving:
        ...
        lock.acquire() 
        ack = int.from_bytes(data,'big') #convert data to integer
...
if __name__ == "__main__":
    ...
    retransmits = send(s,packets,timeout,remotehost,port)
    ...
    output = str(retransmits) + ' ' + str(round((throughput),2)) # format output string as required
    print(output)
    s.close()
